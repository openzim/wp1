...
def _get_zim_file_for_selection(wp10db, selection_id):
    with wp10db.cursor() as cursor:
        cursor.execute(
            "SELECT * FROM zim_tasks WHERE z_selection_id = %s LIMIT 1", (selection_id,)
        )
        db_zim_file = cursor.fetchone()
        return ZimTask(**db_zim_file)
...
class SelectionTest(BaseWpOneDbTest):
    ...
    def _insert_selections(self, selections=None, zim_tasks=None):
        if selections is None:
            selections = [self.selection]
        if zim_tasks is None:
            zim_tasks = [self.zim_file]
        selections = [attr.asdict(s) for s in selections]
        zim_tasks = [attr.asdict(z) for z in zim_tasks]
        ...
        with self.wp10db.cursor() as cursor:
           ...
           VALUES
             (%(z_id)s, %(z_selection_id)s, %(z_zim_schedule_id)s, %(z_status)s, %(z_task_id)s,
              %(z_requested_at)s)
          """,
                zim_tasks,
            )
    ...
    def test_get_next_version_empty_table(self):
        actual = logic_selection.get_next_version(
            self.wp10db, 100, b"text/tab-separated-values"
        )
    ...
    def test_get_next_version_no_builder_match(self):
        ...
        actual = logic_selection.get_next_version(
            self.wp10db, 200, b"text/tab-separated-values"
        )
        self.assertEqual(1, actual)

    def test_get_next_version_no_content_match(self):
        self._insert_selections()
        actual = logic_selection.get_next_version(self.wp10db, 100, b"foo/content-type")
        self.assertEqual(1, actual)
    ...
    def test_object_key_for(self):
        actual = logic_selection.object_key_for(
            "abcd-1234", "text/tab-separated-values", "foo.bar.model"
        )
        self.assertEqual("selections/foo.bar.model/abcd-1234/selection.tsv", actual)

    def test_object_key_for_none_selection_id(self):
        with self.assertRaises(ValueError):
            logic_selection.object_key_for(
                None, "text/tab-separated-values", "foo.bar.model"
            )
    ...
    def test_object_key_for_none_model(self):
        with self.assertRaises(ValueError):
            logic_selection.object_key_for(
                "abcd-1234", "text/tab-separated-values", None
            )
    ...
    def test_url_for_escapes(self):
        ...
        self.assertEqual(
            ...
            "H%C3%A9llo%20W%C3%B8rld.%3F%3F%3F",
            actual,
        )

    def test_object_key_for_selection_with_name(self):
        actual = logic_selection.object_key_for_selection(
            self.selection, "foo.bar.model", name="name"
        )
    ...
    def test_object_key_for_none_content_type_and_name(self):
        actual = logic_selection.object_key_for(
            "abcd-1234", None, "foo.bar.model", name="name"
        )
    ...
    def test_delete_keys_from_storage_single_bytes(self, patched_connect_storage):
        ...
        bucket.delete_objects.assert_called_once_with(
            Delete={"Objects": [{"Key": "object/key/1"}], "Quiet": True}
        )
        self.assertTrue(actual)
    ...
    def test_get_resource_profile(self):
        ...
        for article_count, expected_memory in test_params:
            ...
            self.assertEqual(
                {"cpu": 3, "disk": 21474836480, "memory": expected_memory}, actual
            )
