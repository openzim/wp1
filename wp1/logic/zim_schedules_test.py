...
class LogicZimSchedulesTest(BaseWpOneDbTest):
  ...
  def new_schedule(self, builder_id=123, interval=2, remaining=5):
    return ZimSchedule(
      s_id=str(uuid.uuid4()),
  ...
  def test_list_for_builder(self):
    ...
    s3 = self.new_schedule(builder_id=200, interval=3, remaining=3)
...
def test_get_scheduled_zimfarm_task_from_taskid(self):
    ...
    self.assertIsNone(get_scheduled_zimfarm_task_from_taskid(self.wp10db, 99999))
