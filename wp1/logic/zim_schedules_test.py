...
class LogicZimSchedulesTest(BaseWpOneDbTest):
    ...
    def test_insert_and_get(self):
        ...
        self.assertEqual(schedule.s_interval, fetched.s_interval)
        self.assertEqual(
            schedule.s_remaining_generations, fetched.s_remaining_generations
        )
    ...
    def test_get_scheduled_zimfarm_task_from_taskid_is_none(self):
        ...
        # No schedule yet
        self.assertIsNone(
            get_scheduled_zimfarm_task_from_taskid(self.wp10db, z_task_id)
        )
        # Insert a schedule linked to the zim_file
        schedule = self.new_schedule()
        insert_zim_schedule(self.wp10db, schedule)
        ...
        self.assertIsNone(
            get_scheduled_zimfarm_task_from_taskid(self.wp10db, b"non_existent_task_id")
        )

    @patch("wp1.queues.schedule_recurring_zimfarm_task")
    def test_schedule_future_zimfile_generations(
        self, mock_schedule_recurring_zimfarm_task
    ):
        builder = Builder(
            b_id=b"builder-id",
        ...
        result = schedule_future_zimfile_generations(
            self.redis, self.wp10db, builder, zim_schedule.s_id, scheduled_repetitions
        )
        ...
        with self.wp10db.cursor() as cursor:
            cursor.execute(
                "SELECT * FROM zim_schedules WHERE s_id = %s", (zim_schedule.s_id,)
            )
            ...
            self.assertEqual(
                scheduled_repetitions["email"].encode("utf-8"),
                actual_zim_schedule["s_email"],
            )
            self.assertEqual(
                scheduled_repetitions["repetition_period_in_months"],
                actual_zim_schedule["s_interval"],
            )
            self.assertEqual(
                scheduled_repetitions["number_of_repetitions"],
                actual_zim_schedule["s_remaining_generations"],
            )
    ...
    def test_get_username_by_zim_schedule_id_username_is_none(self):
        ...
        with self.wp10db.cursor() as cursor:
            cursor.execute(
                "INSERT INTO users (u_id, u_username) VALUES (%s, %s)", (user_id, None)
            )
            builder_id = b"builder-123"
            cursor.execute(
                "INSERT INTO builders (b_id, b_name, b_user_id, b_project, b_model, b_params) VALUES (%s, %s, %s, %s, %s, %s)",
    ...
    def test_set_zim_schedule_id_to_zim_task_by_selection_success(self):
        ...
        updated = set_zim_schedule_id_to_zim_task_by_selection(
            self.wp10db, z_selection_id, schedule.s_id
        )
        self.assertTrue(updated)
        with self.wp10db.cursor() as cursor:
            cursor.execute(
                "SELECT z_zim_schedule_id FROM zim_tasks WHERE z_selection_id = %s",
                (z_selection_id,),
            )
            row = cursor.fetchone()
    ...
    def test_set_zim_schedule_id_to_zim_task_by_selection_no_match(self):
        ...
        with self.wp10db.cursor() as cursor:
            cursor.execute(
                "SELECT * FROM zim_tasks WHERE z_selection_id = %s", (z_selection_id,)
            )
            row = cursor.fetchone()
