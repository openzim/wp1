...
class BuilderTest(BaseWpOneDbTest):
  ...
  def test_materialize_builder_with_connections(self):
    ...
    logic_builder.materialize_builder(TestBuilderClass, self.builder,
                                      'text/tab-separated-values', s3, redis,
                                      self.wp10db)
  ...
  def test_materialize_builder_no_update_zim_version(self,
    ...
    try:
      ...
      logic_builder.materialize_builder(TestBuilderClass, self.builder,
                                        'text/tab-separated-values')
      mock_handle_zim_generation.assert_called_once_with(redis,
                                                         self.wp10db,
                                                         self.builder.b_id,
                                                         title=None,
                                                         description=None,
  ...
  def test_handle_zim_generation_long_title(self, mock_utcnow,
                                            mock_get_zimfarm_token):
    redis = MagicMock()
    s3 = MagicMock()
  ...
  def test_handle_zim_generation_with_existing_schedule_update(
    ...
    mock_update_schedule.assert_called_once_with(redis, self.wp10db,
                                                 self.builder, 'Updated Title',
                                                 'Updated Description',
  ...
  def test_handle_zim_generation_create_new_schedule(
    ...
    mock_create_schedule.assert_called_once_with(redis, self.wp10db,
                                                 self.builder, 'New Title',
                                                 'New Description',
  ...
  @patch('wp1.logic.selection.utcnow',
         return_value=datetime.datetime(2022, 12, 25, 0, 1, 2))
  def test_on_zim_file_status_poll_file_ready(self, mock_utcnow, mock_redis,
  ...
  def test_on_zim_file_status_poll_expired(self, mock_redis, mock_connect,
    ...
    with self.wp10db.cursor() as cursor:
      cursor.execute('SELECT z_status, z_updated_at '
                     'FROM zim_tasks WHERE z_selection_id = 1')
      data = cursor.fetchone()
  ...
  def test_auto_handle_zim_generation_cancel_tasks(self, mock_cancel_zim,
    ...
    mock_cancel_zim.assert_has_calls((call(redis, '1abc'), call(redis, '9def')),
                                     any_order=True)
