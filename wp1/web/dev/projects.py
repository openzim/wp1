...
def mark_project_manual_update_time(redis, project_name):
    ...
    ts = (utcnow() + timedelta(seconds=UPDATE_DURATION_SECS)).strftime(
        "%Y-%m-%d %H:%M:%S UTC"
    )
...
def _progress_secs(dt, project_name):
    ...
    secs = (dt - timedelta(seconds=UPDATE_DURATION_SECS - ELAPSED_TIME_SECS)) - utcnow()
...
def get_project_queue_status(redis, project_name):
    ...
    if progress > ELAPSED_TIME_SECS * 3 // 4:
        ...
        if (
            project_name == b"Aesthetics" and progress < int(ELAPSED_TIME_SECS * 1.2)
        ) or (project_name == b"Basic_Income" and progress < BI_DURATION_SECS):
