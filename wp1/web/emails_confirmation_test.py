...
class EmailConfirmationTest(BaseWpOneDbTest):
    ...
    def test_respond_to_zim_task_completed_with_unconfirmed_email(
        self, mock_decrement, mock_notify
    ):
    ...
    @patch("wp1.web.emails.notify_user_for_scheduled_zim")
    @patch("wp1.logic.zim_schedules.decrement_remaining_generations")
    def test_respond_to_zim_task_completed_with_confirmed_email(
        self, mock_decrement, mock_notify
    ):
    ...
    @patch("wp1.web.emails.notify_user_for_scheduled_zim")
    @patch("wp1.logic.zim_schedules.decrement_remaining_generations")
    def test_respond_to_zim_task_completed_with_no_email(
        self, mock_decrement, mock_notify
    ):
    ...
    def test_send_zim_email_confirmation_success(
        ...
        mock_jinja_env.get_template.assert_called_once_with(
            "zim-email-confirmation.html.jinja2"
        )
        ...
        self.assertEqual(
            email_data["subject"], "Confirm Email Notifications for ZIM File - Test ZIM"
        )
