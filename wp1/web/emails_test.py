...
from wp1.web.emails import respond_to_zim_task_completed, send_zim_ready_email
...
class EmailsTest(BaseWpOneDbTest):
  ...
  def test_send_zim_ready_email(self, mock_jinja_env, mock_credentials,
                                mock_post):
    ...
    mock_jinja_env.get_template.assert_called_once_with(
        'generated-scheduled-zim-email.html.jinja2')
    ...
    self.assertEqual(
        email_data['from'],
        'Wikipedia WP 1.0 Project <notifications@mg.wp1.openzim.org>')
    self.assertEqual(email_data['to'], 'testuser <test@example.com>')
    self.assertEqual(email_data['subject'],
                     'Your ZIM File is Ready for Download - Test ZIM')
  ...
  @patch('wp1.web.emails.ENV', 'test')
  @patch('wp1.web.emails.jinja_env')
  def test_send_zim_ready_email_with_defaults(self, mock_jinja_env,
                                              mock_credentials, mock_post):
  ...
  @patch('wp1.web.emails.ENV', 'test')
  @patch('wp1.web.emails.jinja_env')
  def test_send_zim_ready_email_api_error(self, mock_jinja_env,
                                          mock_credentials, mock_post):
  ...
  @patch('wp1.web.emails.ENV', 'test')
  @patch('wp1.web.emails.jinja_env')
  def test_send_zim_ready_email_request_exception(self, mock_jinja_env,
                                                  mock_credentials, mock_post):
  ...
  def test_respond_to_zim_task_completed(self, patched_get, mock_get_username,
                                         mock_send_email):
    ...
    mock_get_username.assert_called_once_with(self.wp10db,
                                              self.zim_schedule.s_id)
    mock_send_email.assert_called_once_with(
        ...
        download_url=
        'https://fake.wasabisys.com/org-kiwix-zimit/wikipedia/foo.zim',
        unsubscribe_url=ANY,
        next_generation_months=3)
  ...
  def test_respond_to_zim_task_completed_no_remaining_generations(
      self, mock_decrement, mock_zimfile_url, mock_get_username,
      mock_send_email):
    ...
    respond_to_zim_task_completed(self.wp10db, self.zim_task,
                                  schedule_no_generations)
    ...
    mock_send_email.assert_called_once_with(
        ...
        None  # Should be None when no remaining generations
    )
  ...
  @patch('wp1.web.emails.zimfarm.zim_file_url_for_task_id')
  @patch('wp1.web.emails.zim_schedules.decrement_remaining_generations')
  def test_respond_to_zim_task_completed_no_title(self, mock_decrement,
                                                  mock_zimfile_url,
                                                  mock_get_username,
                                                  mock_send_email):
    ...
    mock_send_email.assert_called_once_with(
        ...
        download_url='https://download.example.com/test.zim',
        unsubscribe_url=
        'http://test.server.fake/api/v1/zim/unsubscribe-notification?schedule_id=test-schedule-id',
        next_generation_months=3)
  ...
  def test_respond_to_zim_task_completed_includes_unsubscribe_url(
    ...
    self.assertIn(f"schedule_id={self.zim_schedule.s_id.decode('utf-8')}",
                  unsubscribe_url)
