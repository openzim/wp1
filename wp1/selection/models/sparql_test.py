...
class SparqlBuilderTest(BaseWpOneDbTest):
  ...
  """
    ...
    french_query = cats_uk_us_after_1950.replace("en.wikipedia.org", "fr.wikipedia.org")
    ...
    json_return_value = {
        "results": {
            "bindings": [
                {
                    "cat": {"value": "Sparkles"},
                    "birth": {"value": "1960-01-01"},
                    "url": {"value": "https://en.wikipedia.org/wiki/Foo"},
                },
                {
                    "cat": {"value": "Sprinkles"},
                    "birth": {"value": "1965-01-01"},
                    "url": {"value": "https://en.wikipedia.org/wiki/B%C3%A5r"},
                },
            ]
        }
    }
            ...
            "bindings": [
                {
                    "cat": {"value": "Sparkles"},
                    "birth": {"value": "1960-01-01"},
                    "url": {"value": "https://fr.wikipedia.org/wiki/Foo"},
                },
                {
                    "cat": {"value": "Sprinkles"},
                    "birth": {"value": "1965-01-01"},
                    "url": {"value": "https://fr.wikipedia.org/wiki/B%C3%A5r"},
                },
            ]
        }
    }
    ...
    def test_build(self, mock_requests):
        ...
        mock_requests.post.assert_called_once_with(
            ...
            data={"query": self.cats_uk_us_after_1950, "format": "json"},
        )
    ...
    def test_build_french_wikipedia(self, mock_requests):
        ...
        mock_requests.post.assert_called_once_with(
            "https://query.wikidata.org/sparql",
            headers={"User-Agent": "WP 1.0 bot 1.0.0/Audiodude <audiodude@gmail.com>"},
            data={"query": self.french_query, "format": "json"},
        )
        response.json.assert_called_once()
        self.assertEqual("Foo\nBår".encode("utf-8"), actual)
    ...
    def test_build_wrong_content_type(self):
        with self.assertRaises(Wp1FatalSelectionError):
            actual = self.builder.build(
                None, query=self.cats_uk_us_after_1950, queryVariable="cat"
            )
    ...
    def test_validate_no_parse(self):
        ...
        for query in invalid_queries:
            ...
            self.assertEqual(
                ("", query, ["Could not parse query, are you sure it's valid SPARQL?"]),
                actual,
            )
    ...
    def test_validate_missing_prefix(self):
        ...
        actual = self.builder.validate(
            project="en.wikipedia.org", query=query, queryVariable="foo"
        )
