...
class WikiProjectTest(BaseWpOneDbTest):
    ...
    def setUp(self):
        ...
        all_articles = [water_articles, fire_articles, wind_articles, earth_articles]
        ...
        with self.wp10db.cursor() as cursor:
            ...
            cursor.executemany(
                "INSERT INTO projects (p_project, p_timestamp) VALUES (%s, 0)", projects
            )
    ...
    def test_validate(self):
        ...
        self.assertEqual(
            (
                ["Water Elements", "Fire", "Wind", "Èarth"],
                ["Lava", "Stone"],
                ["Not all given projects exist"],
            ),
            actual,
        )

    def test_validate_all_valid(self):
        params = {"include": ["Water Elements", "Fire"], "exclude": ["Wind", "Èarth"]}
        actual = self.builder.validate(wp10db=self.wp10db, **params)
        self.assertEqual((["Water Elements", "Fire", "Wind", "Èarth"], [], []), actual)

    def test_validate_all_invalid(self):
        params = {"include": ["Lava"], "exclude": ["Stone"]}
        actual = self.builder.validate(wp10db=self.wp10db, **params)
        self.assertEqual(
            ([], ["Lava", "Stone"], ["Not all given projects exist"]), actual
        )
