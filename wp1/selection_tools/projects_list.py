...
def build_translated_titles(
    ...
    titles = set()
    with title_fp.open(encoding="utf-8") as f:
        for line in f:
...
def build_translated_list(
    ...
    titles = build_translated_titles(src, lang_code, scores_fp, langlinks_fp)
    content = "\n".join(titles)
    if content:
        content += "\n"
    (out_dir / src.name).write_text(content, encoding="utf-8")
