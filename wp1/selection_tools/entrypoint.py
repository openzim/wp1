...
def save_pagecount_results(
    ...
    if results_fp.exists():
        with results_fp.open(encoding="utf-8") as f:
            for line in f:
                parts = line.strip("\n").split("\t")
                if len(parts) != 2 or not parts[0] or not parts[1].isdigit():
                    logger.error(
                        f"unable to parse existing title counts from {results_fp}"
                    )
                    continue
    ...
    with results_fp.open("w", encoding="utf-8") as f:
...
def build_selections(lang_code: str, start: int, data_dir: pathlib.Path):
    ...
    pageviews_cache_fp = data_dir / f"pageviews_{wiki}"
    save_pagecount_results(pageviews_cache_fp, *page_count_results)
    shutil.copy(pageviews_cache_fp, pageviews_tsv_fp)
    # Free up the memory used by the page count results as this could get very big
    page_count_results = []

    logger.info("Gathering pages...")
    readme_lines.append("pages.tsv: page_id page_title page_size is_redirect\n")
...
def main():
    ...
    build_parser.add_argument(
        ...
        type=int,
        default=2017,
        help="First year of pagecount dumps to consider (default: %(default)s).",
    )
    build_parser.add_argument(
        "--data-dir",
        type=pathlib.Path,
    ...
    if args.command == "build":
        build_selections(args.lang, args.start, args.data_dir)
