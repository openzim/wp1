...
def build_custom_selections(
    ...
    tunisie_titles = sorted(tunisie) + ["Portail:Tunisie/Index thématique"]
    with (custom_dir / "tunisie.tsv").open("w", encoding="utf-8") as f:
        write_tsv_rows_to_file(f, [(title,) for title in tunisie_titles])
    ...
    medicine_rows = sorted((title,) for title in set(translated) | native)
    with (custom_dir / "medicine.tsv").open("w", encoding="utf-8") as f:
        write_tsv_rows_to_file(f, medicine_rows)
