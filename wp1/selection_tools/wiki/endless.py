...
def build_custom_selection(lang_code: str, custom_dir: pathlib.Path) -> None:
    base_sel = DATA_DIR / "endless" / lang_code / "base_selection"
    if not base_sel.is_file():
        return

    base_path = custom_dir.parent / base_sel.read_text(encoding="utf-8").strip()
    ...
    with (custom_dir / "endless.tsv").open("w", encoding="utf-8") as f:
        write_tsv_rows_to_file(f, [(title,) for title in combined])
